<template>
  <PageComponent>
    <template v-slot:header>
      <div class="flex justify-between items-center">
        <h1 class="text-3xl font-bold text-gray-900">
          {{ route.params.id ? model.title : "Tạo cuộc khảo sát" }}
        </h1>

        <button v-if="route.params.id" type="button" @click="deleteSurvey()"
                class="bg-red-500 text-white py-2 px-3 rounded-md hover:bg-red-600">
          <svg
    ...
    <div v-if="surveyLoading" class="flex justify-center">Đang tải...</div>
      ...
      <div class="shadow sm:rounded-md sm:overflow-hidden">
        ...
        <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
          ...
          <div>
            ...
            <input type="text" name="title" id="title" v-model="model.title" autocomplete="survey_title"
                   class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full rounded-md shadow-sm sm:text-sm border-gray-300"/>
          </div>
          <!--/ Title -->
          ...
          <div>
            ...
            <div class="mt-1">
              <textarea name="description" id="description" rows="3" v-model="model.description"
                        ...
                        class="focus:ring-indigo-500 focus:border-indigo-500 block w-full rounded-md shadow-sm sm:text-sm border-gray-300"
                        placeholder="Mô tả cuộc khảo sát của bạn"/>
            </div>
          </div>
          <!--/ Description -->
          ...
          <!-- Expire Date -->
          <div>
            <label for="expire_date" class="block text-sm font-medium text-gray-700">Hạn sử dụng</label>
          ...
          <div class="flex items-start">
            <div class="flex items-center h-5">
              <input type="checkbox" name="status" id="status" v-model="model.status" autocomplete="survey_status"
                     class="focus:ring-indigo-500 h-4 w-4 text-indigo-500 rounded border-gray-300"/>
            </div>
        ...
        <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
          ...
          <div v-if="!model.questions.length" class="text-center text-gray-600">
            Bạn không có bất kỳ câu hỏi nào được tạo
          </div>
          <div v-for="(question, index) in model.questions" :key="question.id" class="text-center text-gray-600">
            <QuestionEditor
              :question="question"
              :index="index"
...
function saveSurvey(){
  store.dispatch("saveSurvey", model.value).then(({ data }) => {
    store.commit('notify', {
      type: "success",
      message: 'Bản khảo sát đã được cập nhật thành công!'
    })
    router.push({
      name: "SurveyView",
...
function deleteSurvey(){
  if ( confirm(`Bạn có chắc chắn muốn xóa cuộc khảo sát này? Không thể hoàn tác thao tác !!`) ) {
